...
echo "ngen-j job --last-success       # 10 job terakhir yang success"
echo "ngen-j job --last-failure       # 10 job terakhir yang fail"
echo "ngen-j build <nama-job>         # Trigger build"
echo "ngen-j build <nama-job> --param REF_NAME=develop REF_TYPE=branch # Multiple params"
echo "ngen-j build <nama-job> --param=REF_NAME=develop # Format alternatif"
echo "ngen-j log <nama-job> <build-no> # Get console output"
echo "ngen-j get-xml <nama-job>       # Get job XML config"
echo "ngen-j create <nama-job> <xml>  # Create job from XML"
echo "ngen-j delete <nama-job>        # Delete job (with confirmation)"
echo "ngen-j delete <nama-job> --force # Delete job without confirmation"
echo "ngen-j plugin list              # List semua plugins terinstall"
echo "ngen-j plugin list --format json --output plugins.json  # Export ke JSON"
echo "ngen-j plugin list --format csv --output plugins.csv   # Export ke CSV"
echo "ngen-j plugin install git       # Install plugin git"
echo "ngen-j plugin uninstall git     # Uninstall plugin git"
echo "ngen-j cred list                # List semua credentials"
echo "ngen-j cred create              # Create credential (interactive)"
echo "ngen-j cred create --type username_password --id my-cred --username user --password pass  # Non-interactive"
echo "ngen-j cred delete my-cred      # Delete credential"
echo
